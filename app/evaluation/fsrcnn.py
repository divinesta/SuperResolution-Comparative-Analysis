"""FSRCNN evaluation through the project's fixed super-resolution protocol."""
...
from app.deep_learning.checkpoints import FSRCNN_CHECKPOINTS
from app.deep_learning.fsrcnn import (
    FSRCNN,
    pil_to_luminance_tensor,
    tensor_and_lr_to_pil,
)
...
class FSRCNNEvaluationConfig:
    """Settings shared by one dataset-scale FSRCNN GPU evaluation."""
...
def _measure_cuda_forward(
    ...
    if input_tensor.device.type != "cuda":
        raise ValueError("FSRCNN GPU timing requires a CUDA input tensor.")
...
def evaluate_fsrcnn_image(
    ...
    *,
    ...
    """Evaluate one prepared HR/LR pair with a loaded FSRCNN model."""
    if device.type != "cuda":
        raise ValueError("The full FSRCNN GPU evaluator requires a CUDA device.")
    if model.scale_factor != config.scale:
        raise ValueError(
            f"Loaded FSRCNN is x{model.scale_factor}, "
            f"but evaluation requested x{config.scale}."
        )
    ...
    input_tensor = pil_to_luminance_tensor(lr_image, device)
    output_tensor, timing, peak_memory_mb = _measure_cuda_forward(
        model,
        input_tensor,
        config,
    )
    native_reconstruction = tensor_and_lr_to_pil(output_tensor, lr_image)
    ...
    if native_reconstruction.size != expected_native_size:
        raise RuntimeError(
            f"FSRCNN x{config.scale} returned {native_reconstruction.size}; "
            f"expected {expected_native_size}."
        )

    aligned = align_reconstruction_to_target(native_reconstruction, reference_hr.size)
    metrics = calculate_quality_metrics(reference_hr, aligned.image, border=config.scale)
    if sr_output_dir is not None:
        output_name = f"{hr_image_path.stem}_fsrcnn_x{config.scale}.png"
        save_image(aligned.image, Path(sr_output_dir) / output_name)

    provenance = FSRCNN_CHECKPOINTS[config.scale]
    ...
    return {
        ...
        "colour_policy": "limited_bt601_y_model_with_bicubic_cbcr",
        "timing_device": "gpu",
