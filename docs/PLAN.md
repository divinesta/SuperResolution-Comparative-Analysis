...
**In progress (6 September 2026):** IMDN is selected as the neural component.
The transparent weighted-fusion implementation and its tests are complete.
Weight selection is fixed in `docs/notes/phase5_fusion_protocol.md` and run by
`notebooks/14_phase5_fusion_weight_selection_colab.ipynb`. Final benchmark
evaluation will begin only after the validation run selects one fixed weight.
