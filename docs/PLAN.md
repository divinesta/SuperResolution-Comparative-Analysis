...
FLOPs reporting remains to be completed during Phase 4.
...
**Current progress (5 September 2026):** The numerical comparison has begun.
All 48 method/dataset/scale summary groups are present. The detailed NEDI,
FSRCNN, and IMDN files each contain 657 validated image-scale records and
reproduce their final summaries. Combined tables and numerical figures are
generated by `notebooks/12_phase4_comparative_analysis.ipynb`. The remaining
work is the qualitative image comparison, FLOPs reporting, and retrieval of
the final detailed Bicubic records if available. Bicubic latency was measured
on Google Colab's default CPU, whose exact model was not recorded; NEDI used an
AWS m7i.2xlarge CPU instance, while FSRCNN and IMDN used a Tesla T4 GPU.
The remaining FLOPs and selected reconstruction export are prepared in
`notebooks/13_phase4_flops_and_visuals_colab.ipynb`.
